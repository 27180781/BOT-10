...
engine = None; SessionLocal = None; Base = declarative_base()
...
google_api_key = None
gemini_model = None
chat_session = None # <<<--- מאתחלים את משתנה השיחה הגלובלי כ-None
try:
    ...
    if not google_api_key_from_env: print("ERROR: GOOGLE_API_KEY environment variable not set.")
        ...
        model_name = 'gemini-2.0-flash'; # או המודל שבחרת שעובד
        gemini_model = genai.GenerativeModel(model_name); # יוצרים את המודל פעם אחת
...
def health_check(): # ... (ללא שינוי) ...
    print("--- Reached /health route ---"); db_status = "OK" if engine and SessionLocal else "Error"; llm_status = "OK" if google_api_key and gemini_model else "Error"
    return jsonify({"status": "OK", "message": "Backend is running", "db_connection": db_status, "llm_configured": llm_status}), 200
@app.route('/db-test')
...
# --- נתיב API לטיפול בצ'אט - *** מעודכן לשיחה מתמשכת *** ---
@app.route('/api/chat', methods=['POST'])
def handle_chat():
    global chat_session # <<<--- מאפשרים לפונקציה לגשת ולשנות את המשתנה הגלובלי

    # 1. Parse User Input (ללא שינוי)
    # ... (קוד קריאת הודעת המשתמש) ...
    ...
    # 3. Initialize Chat Session if needed <<<--- שינוי כאן
    if chat_session is None:
        try:
            print("--- Starting new chat session ---")
            chat_session = gemini_model.start_chat(history=[]) # מתחילים שיחה חדשה וריקה
    ...
    # 5. Format Prompt/Message (נשלב הקשר + הודעה)
    message_to_send = f"{context}\n\nשאלת המשתמש:\n{user_message}"
    print(f"--- Message being sent to chat session:\n{message_to_send[:500]}...")

    # 6. Send to Gemini via Chat Session <<<--- שינוי כאן
    ...
    try:
        ...
        chat_session = None # אם יש שגיאה בתקשורת, נאפס את השיחה כדי שהפעם הבאה תתחיל חדשה
