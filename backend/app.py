...
# --- Initialize DB (Call init_db only AFTER successful engine creation attempt) ---
# מיקום נכון - אחרי ה-try-except של יצירת ה-engine
if engine:
   init_db()
# --------------------------------------------------------------------
...
google_api_key = None
gemini_model = None
chat_session = None # Initialize global chat session placeholder
try:
...
def health_check():
    ...
    return jsonify({
        ...
        "db_connection": db_status, "llm_configured": llm_status
    }), 200
...
def handle_chat():
    ...
    # 3. RAG - Retrieve Context (DB Lookup)
    ...
    # 4. Format Prompt using Template
    try:
    ...
    # 5. Send to Gemini (Using generate_content for stability)
    ...
    try:
        ...
        if response.parts: llm_reply = response.text
             ...
             llm_reply = f"שגיאה: התגובה נחסמה או לא הושלמה. סיבה: {response.candidates[0].finish_reason}"; print(f"LLM response blocked or incomplete. Reason: {response.candidates[0].finish_reason}")
