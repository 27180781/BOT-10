...
if not DATABASE_URL:
    ...
    try:
        ...
        # --- Initialize DB (Call init_db if engine was created) ---
        if engine:
           init_db() # <<<--- Re-enabled (uncommented)
...
def handle_chat():
    ...
    # 5. Send to Gemini (Using generate_content)
    ...
    try:
        ...
        if response.parts:
             ...
             llm_reply = "קיבלתי תשובה ריקה מהמודל." # Handle case where response.text might be missing
             print(f"Warning: Received potentially empty response from LLM. Full response object: {response}")
