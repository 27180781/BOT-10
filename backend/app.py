# backend/app.py (Re-implementing Conversational Chat)
...
load_dotenv()
print("--- Flask app starting (Conversational Version) ---") # Updated print
...
def seed_data(db_session: Session): #... (No change) ...
...
if not DATABASE_URL: print("CRITICAL ERROR: DATABASE_URL environment variable not set."); db_setup_error = "DATABASE_URL not set"
    ...
    try: print(f"--- Attempting create_engine with URL: '{DATABASE_URL}' ---"); engine = create_engine(DATABASE_URL); print(f"--- Database engine object CREATED: {engine} ---"); SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine); print("--- SessionLocal created ---");
        if engine: init_db() # Call DB init
...
google_api_key = None
gemini_model = None
chat_session = None # <<<--- Global chat session object
...
try:
    ...
    if not google_api_key_from_env: print("ERROR: GOOGLE_API_KEY environment variable not set."); gemini_setup_error = "GOOGLE_API_KEY not set"
        ...
        model_name = 'gemini-2.0-flash'; print(f"--- Attempting genai.GenerativeModel('{model_name}') ---");
        gemini_model = genai.GenerativeModel(model_name); # Create model object once
...
@app.route('/')
def home(): # ... (No change) ...
    print("--- Reached / route ---"); return "Chatbot Backend Running!"
@app.route('/health')
def health_check(): # ... (No change) ...
    print("--- Reached /health route ---"); db_status = "OK" if engine and SessionLocal else f"Error ({db_setup_error or 'Unknown'})"; llm_status = "OK" if google_api_key and gemini_model else f"Error ({gemini_setup_error or 'Unknown'})"; return jsonify({"status": "OK", "message": "Backend is running", "db_connection_setup": db_status, "llm_configured": llm_status}), 200
@app.route('/db-test')
def db_test(): # ... (No change) ...
...
# --- /api/chat Route - *** UPDATED BACK TO CONVERSATIONAL *** ---
@app.route('/api/chat', methods=['POST'])
def handle_chat():
    global chat_session # Use the global chat session object
    ...
    # 3. Initialize Chat Session if needed <<<--- Re-enabled this logic
    if chat_session is None:
        try:
            print("--- Starting new chat session ---")
            chat_session = gemini_model.start_chat(history=[]) # Start a fresh chat
    ...
    # 4. RAG - Retrieve Context (No change)
    ...
    # 5. Format Prompt/Message (No change)
    try: final_prompt = PROMPT_TEMPLATE.format(context=context, user_message=user_message); print(f"--- Final prompt for Gemini (to be sent via send_message):\n{final_prompt[:500]}...")
    ...
    # 6. Send to Gemini via Chat Session <<<--- Using send_message again
    llm_reply = "Error: LLM did not respond."
    try:
        print("--- Sending message via chat.send_message() ---")
        response = chat_session.send_message(final_prompt) # Send the augmented prompt to the session
        ...
        if response.parts: llm_reply = response.text
             ...
             try: llm_reply = response.text
             except ValueError: llm_reply = "קיבלתי תשובה ריקה או חסומה מהמודל."; print(f"Warning: Received empty/blocked response. Response: {response}")
             except Exception as e_resp_text: llm_reply = "שגיאה בעיבוד תשובת המודל."; print(f"Error accessing response.text: {e_resp_text}. Response: {response}")
